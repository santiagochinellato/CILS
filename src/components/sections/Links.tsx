      ...
      <div className="container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {links.map((l, i) => {
            ...
            const isLight = variant.startsWith('bg-white');
            return (
              <a
                key={l.href}
                href={l.href}
                ...
                className={cn('group rounded-xl p-5 shadow-card flex flex-col gap-4 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-accent1 focus:ring-offset-2', variant)}
                aria-label={`${l.title} - abrir en nueva pestaña`}
              >
                <div className="flex items-center gap-3">
                  <Icon name={l.icon as any} size={26} context={isLight ? 'default' : 'footer'} />
                  <h2 className="text-lg font-semibold font-montserrat flex-1 leading-snug">{l.title}</h2>
                  <Icon name="arrowRight" size={18} context={isLight ? 'default' : 'footer'} className="opacity-70 group-hover:translate-x-1 transition-transform duration-200" />
                </div>
                <p className={cn('text-sm leading-relaxed', isLight ? 'text-textLight dark:text-white/70' : 'text-white/90')}>{l.description}</p>
                <span className={cn('inline-flex items-center gap-1 text-xs font-medium mt-auto', isLight ? 'text-primary dark:text-accent1' : 'text-white/90')}>
